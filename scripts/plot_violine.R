...
gg <- DimPlot(object = seuratobj, reduction = "tsne", group.by="orig.ident")
ggsave(gg, file = file.path(path_advplots, "plot_tsne_bysample.pdf"), width = 12, height = 7)
gg <- DimPlot(object = seuratobj, reduction = "pca", group.by="orig.ident")
ggsave(gg, file = file.path(path_advplots, "plot_pca_bysample.pdf"), width = 12, height = 7)
gg <- DimPlot(object = seuratobj, reduction = "pca")
ggsave(gg, file = file.path(path_advplots, "plot_pca_bycluster.pdf"), width = 12, height = 7)
...
Misc(object = seuratobj, slot = "misc")  <- list(snakemake)
